...
class DisplayController:
    ...
    def start_spotify(self, account: Optional[str] = None,
        ...
        self._launch_spotify_thread(auths)
        return {'status': 'started', 'account': ', '.join(auths.keys())}
    ...
    def _spotify_loop(self, auths: dict):
        ...
        *auths* is a dict of account_name -> SpotifyOAuth.
        ...
        try:
            ...
            clients = {name: _spotipy.Spotify(auth_manager=a) for name, a in auths.items()}
...
def main():
    ...
    if sp_cfg.get('client_id') and sp_cfg.get('client_secret'):
        # Use http://<host>:<port>/callback so OAuth lands back on this server.
